...
def measure_time(sort_function, data):
    ...
    sort_function(data[:])  # Use a copy to avoid modifying the original dataset
    return (time.time() - start_time) * 1000  # Convert to milliseconds

def run_experiments(sizes, runs=3):
    results = []
    for size in sizes:
        bubble_times = []
        heap_times = []
...
if __name__ == "__main__":
    ...
    print("Experiment completed. Results saved to sorting_results.csv.")
