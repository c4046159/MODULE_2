...
if __name__ == "__main__":
    ...
    n = 5  # Number of times to run the full experiment
    for i in range(n):
        print(f"Running experiment {i+1} of {n}")
        experiment_results = run_experiments(dataset_sizes)
        filename = f"results_run_{i+1}.csv"  # Create  Unique filename for each run
        save_results_to_csv(experiment_results, filename)
        print(f"Experiment {i+1} completed. Results saved to {filename}")
    print("All experiments are now completed.")
